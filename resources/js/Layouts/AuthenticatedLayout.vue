<template>
    ...
    <!-- Impersonation Banner (fixed top, pushes layout down when active) -->
    <ImpersonationBanner />
    <!-- Session Idle Timeout Modal -->
    <SessionTimeoutModal />

    <div :class="['h-screen bg-transparent flex overflow-hidden relative z-10', $page.props.impersonation?.active ? 'pt-[52px]' : '']">
        ...
        <aside :class="[
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <!-- Super Administrator: Impersonate Users -->
                <Link v-if="$page.props.auth.roles.includes('super_administrator')" :href="route('admin.impersonate.index')" :class="navClass('admin.impersonate.*')" title="Impersonate User">
                    <svg :class="['w-5 h-5 transition-all duration-300', sidebarCollapsed ? 'md:mr-0 mr-3' : 'mr-3']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    <span :class="[sidebarCollapsed ? 'md:hidden' : '']">Impersonate User</span>
                </Link>
            </nav>
