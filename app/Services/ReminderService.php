        ...
        if ($user->primary_category === 'Client') {
            ...
            foreach ($pendingRequests as $req) {
                ...
                $addTask([
                    ...
                    'due_date' => $req->deadline ? \Carbon\Carbon::parse($req->deadline)->format('Y-m-d') : null,
                    'days_diff' => $daysDiff,
                    'priority' => 'medium',
                    'status' => 'Submitted',
            ...
            foreach ($unpaidQuotations as $quot) {
                ...
                if (!$hasPayment) {
                    ...
                    $addTask([
                        ...
                        'due_date' => $quot->valid_until ? \Carbon\Carbon::parse($quot->valid_until)->format('Y-m-d') : null,
                        'days_diff' => $daysDiff,
                        'priority' => 'high',
            ...
            if ($instructorIntakes->isNotEmpty()) {
                ...
                foreach ($unmarkedSubmissions as $sub) {
                    ...
                    $addTask([
                        ...
                        'due_date' => ($sub->assignment && $sub->assignment->due_date) ? \Carbon\Carbon::parse($sub->assignment->due_date)->format('Y-m-d') : null,
                        'days_diff' => $daysDiff,
                        'priority' => 'high',
            ...
            if ($user->hasAnyRole(['language_expert', 'part_time_staff', 'secretary'])) {
                ...
                foreach ($assignments as $asg) {
                    ...
                    $addTask([
                        ...
                        'due_date' => ($asg->serviceRequest && $asg->serviceRequest->deadline) ? \Carbon\Carbon::parse($asg->serviceRequest->deadline)->format('Y-m-d') : null,
                        'days_diff' => $daysDiff,
